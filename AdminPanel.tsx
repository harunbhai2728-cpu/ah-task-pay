...
export function AdminPanel() {
    ...
    try {
      ...
      if (tx.type === 'deposit') {
        await adminDb.from('profiles').update({ 
          ...
          pendingDepositBalance: (userRef.data.pendingDepositBalance || 0) - tx.amount
        }).eq('id', tx.userId);
      } else if (tx.type === 'withdrawal') {
        await adminDb.from('profiles').update({
          pendingEarningBalance: (userRef.data.pendingEarningBalance || 0) - tx.amount
        }).eq('id', tx.userId);
      }
      ...
      console.error("Deposit Approve Error details:", err);
  ...
  const handleApproveAd = async (ad: any) => {
    try {
      ...
      alert(err.message || 'Error approving ad');
    }
  };
  ...
  const handleRejectAd = async (ad: any) => {
    try {
      ...
      if (userSnap.data) {
        await adminDb.from('profiles').update({ depositBalance: (userSnap.data.depositBalance || 0) + ad.price }).eq('id', ad.userId);
      }

      alert('Advertisement rejected and refunded');
      fetchAdminData();
    } catch (err: any) {
      alert(err.message || 'Error rejecting ad');
    }
  };
  ...
  const handleOpenEditModal = (u: UserProfile) => {
    ...
    setEditDisplayName(u.displayName || (u as any).name || '');
  };
  ...
  const handleSaveUserDetails = async () => {
    ...
    try {
      ...
      if (data.warning) {
        ...
        if (editPassword) {
          alert("পাসওয়ার্ড সফলভাবে পরিবর্তন করা হয়েছে! (Password changed successfully!)\n\nপুরানো পাসওয়ার্ড আর কাজ করবে না। এখন থেকে নতুন পাসওয়ার্ড দিয়ে আইডিতে লগইন করা সম্ভব হবে।");
        } else {
          alert("ব্যবহারকারীর তথ্য সফলভাবে পরিবর্তন করা হয়েছে! (User credentials updated successfully!)");
        }
      }
      setEditingUser(null);
      fetchAdminData();
  ...
  const handleApproveDeleteJob = async (job: any) => {
    try {
      ...
      alert("Job delete request approved, funds refunded.");
      fetchAdminData();
    } catch (error) {
      console.error(error);
      alert("Failed to approve job deletion.");
    }
  };
  ...
  const handleRejectDeleteJob = async (jobId: string) => {
    try {
      ...
      alert("Job delete request rejected, job is open again.");
      fetchAdminData();
    } catch (error) {
       console.error(error);
       alert("Failed to reject job deletion.");
    }
  };
  ...
  const handleRejectJobPending = async (job: any) => {
    try {
      ...
      alert("Job rejected and full funds refunded successfully.");
      fetchAdminData();
    } catch (error: any) {
      console.error(error);
  ...
  const totalEarningBalance = users.reduce((acc, u) => acc + (u.earningBalance || 0), 0);
  const totalDepositBalance = users.reduce((acc, u) => acc + (u.depositBalance || 0), 0);
  const totalHeld = users.reduce((acc, u) => acc + (u.heldBalance || 0), 0);
  ...
  const completedJobsCount = jobs.reduce((acc, j) => acc + (j.completedCount || 0), 0);
      ...
      <div className="flex bg-white dark:bg-slate-800 p-2 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm overflow-x-auto gap-2 transition-colors">
        {(['users', 'jobs', 'transactions', 'submissions', 'tickets', 'ads', 'redeem_codes', 'settings'] as const).map(tab => (
          <button
            ...
            className={cn(
              "flex-1 md:flex-none flex items-center justify-center gap-2 px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all whitespace-nowrap",
          ...
          >
            {tab === 'redeem_codes' ? 'Redeem Codes' : tab}
          </button>
        ))}
      </div>
              ...
              {sortedJobs.map(job => (
                <div key={job.id} className="flex flex-col md:flex-row items-start md:items-center justify-between p-4 bg-gray-50 dark:bg-slate-700/50 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm dark:shadow-none gap-4 transition-colors">
                   <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-gray-500 dark:text-slate-400 uppercase tracking-widest font-black">
                        By: {job.posterName || job.posterId.slice(0,8)} | Status: {job.status}
                      </p>
                   </div>
                  ...
                  {sub.status === 'pending' && (
                    <div className="flex gap-4 pt-4 border-t border-gray-200 dark:border-slate-700 transition-colors">
                       <button 
                         ...
                         className="flex-1 py-4 bg-green-500 text-white rounded-2xl font-black uppercase tracking-widest text-sm shadow-lg dark:shadow-none shadow-green-100 hover:bg-green-600 transition-all flex items-center justify-center gap-2"
                       >
                          <CheckCircle2 className="w-5 h-5" />
                          Approve Proof
                       </button>
                       <button 
                         ...
                         className="flex-1 py-4 bg-red-500 text-white rounded-2xl font-black uppercase tracking-widest text-sm shadow-lg dark:shadow-none shadow-red-100 hover:bg-red-600 transition-all flex items-center justify-center gap-2"
                       >
                          <XCircle className="w-5 h-5" />
                          Reject Proof
                       </button>
