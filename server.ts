...
async function startServer() {
  ...
  // Sync system config from Supabase cloud on startup to prevent automatic resetting
  try {
    ...
    if (superadminProfile && superadminProfile.warning) {
      try {
        const cloudConfig = JSON.parse(superadminProfile.warning);
        if (cloudConfig && cloudConfig.id === 'config') {
          console.log("Durable cloud system_config loaded successfully from Supabase backup.");
          const localStore = getDataStore();
          localStore.system_config = [cloudConfig];
          saveDataStore(localStore);
        }
      } catch (jsonErr) {
        // Not JSON formatted or other data
      }
    }
  } catch (err) {
    console.error("Error syncing system_config from cloud on start:", err);
  }

  // Check if public.redeem_codes table exists and is available on Supabase
  try {
  ...
  // Check if public.system_settings table exists and is available on Supabase
  try {
  ...
  app.post('/api/proxy', async (req, res) => {
    try {
      ...
      const { method, table, args, eq, eqs, single, match, maybeSingle, order, limit, in: inArgs, or: orArgs } = req.body;
      ...
      if (table === 'system_config' || table === 'tickets' || table === 'advertisements') {
          ...
          if (!isAdmin) {
              ...
              if (table === 'advertisements') {
                  if (method === 'select') {
                      // Filter if filtering by userId
                      if (eq && eq[0] === 'userId' && eq[1] !== user.id) {
                          items = items.filter((x: any) => x.userId === user.id);
                      }
                  } else if (method === 'insert') {
      ...
      if (table === 'jobs') {
          ...
          if (method === 'insert' && args?.[0]) {
              ...
              for (const jobItem of tArgs) {
                  ...
                  let feePercent = 10;
                  const store = getDataStore();
                  if (store.system_config && store.system_config[0]) {
                      feePercent = Number(store.system_config[0].jobPostingFee || 10);
                  }
                  ...
                  if (idVal) {
                      ...
                      if (submission && submission.status === 'pending') {
                          ...
                          if (jobData) {
                              ...
                              subProofExtra.rejectionReason = updatedSub.rejectionReason || '';
  ...
  app.get('/api/admin/data', async (req, res) => {
    try {
        ...
        const safeConfig = store.system_config?.[0] || null;
