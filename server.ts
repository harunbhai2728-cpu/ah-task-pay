...
async function startServer() {
  ...
  const runStartupChecks = async () => {
    try {
      ...
      if (!checkErr) {
        ...
        console.log(
          "Supabase redeem_codes table not found/available, using pure emulated fallback. Check message:",
          checkErr.message,
        );
      }
    } catch (err) {
      console.log(
        "Supabase redeem_codes check failed, using pure emulated fallback.",
      );
    }
  ...
  app.post("/api/proxy", async (req, res) => {
    ...
    try {
      ...
      if (
        ...
        if (activeProxyLocks.has(writeLockKey)) {
          return res
            ...
            .json({
              error:
                "Your transaction write is already processing. Please wait.",
            });
        }
      ...
      if (!isAdmin) {
        if (table === "profiles") {
          if (
            ...
            /* Let it query others for displayName maybe? but wait, no! */
      ...
      if (table === "system_config") {
        if (!isAdmin && method !== "select")
          return res
            .status(403)
            .json({ error: "Forbidden settings modification" });
        ...
        if (method === "select") {
            ...
            return res.json({
              data: {
                ...
                transferEarningToDepositFee:
                  data.transfer_earning_deposit_fee || 0,
                transferDepositToEarningFee:
                  data.transfer_deposit_earning_fee || 10,
                ...
                referralValidationCriteria:
                  data.referral_validation_criteria ?? 1,
          ...
          if (input.minDeposit !== undefined)
            mappedUpdate.min_deposit = input.minDeposit;
          if (input.minWithdraw !== undefined)
            mappedUpdate.min_withdraw = input.minWithdraw;
          if (input.withdrawalFee !== undefined)
            mappedUpdate.withdrawal_fee = input.withdrawalFee;
          if (input.jobPostingFee !== undefined)
            mappedUpdate.job_service_charge = input.jobPostingFee;
          if (input.bkashNumber !== undefined)
            mappedUpdate.official_bkash = input.bkashNumber;
          if (input.bkashMethod !== undefined)
            mappedUpdate.bkash_method = input.bkashMethod;
          if (input.nagadNumber !== undefined)
            mappedUpdate.official_nagad = input.nagadNumber;
          if (input.nagadMethod !== undefined)
            mappedUpdate.nagad_method = input.nagadMethod;
          if (input.transferEarningToDepositFee !== undefined)
            mappedUpdate.transfer_earning_deposit_fee =
              input.transferEarningToDepositFee;
          if (input.transferDepositToEarningFee !== undefined)
            mappedUpdate.transfer_deposit_earning_fee =
              input.transferDepositToEarningFee;
          if (input.loginTitle !== undefined)
            mappedUpdate.login_title = input.loginTitle;
          if (input.loginBannerUrl !== undefined)
            mappedUpdate.login_banner_url = input.loginBannerUrl;
          if (input.referralBonusAmount !== undefined)
            mappedUpdate.referral_bonus_amount = input.referralBonusAmount;
          if (input.referralValidationCriteria !== undefined)
            mappedUpdate.referral_validation_criteria =
              input.referralValidationCriteria;
          if (input.referralValidityDays !== undefined)
            mappedUpdate.referral_validity_days = input.referralValidityDays;
          if (input.campaignEndDate !== undefined)
            mappedUpdate.campaign_end_date =
              input.campaignEndDate === "" ? null : input.campaignEndDate;
          if (input.target1Referrals !== undefined)
            mappedUpdate.target_1_referrals = input.target1Referrals;
          if (input.target1Reward !== undefined)
            mappedUpdate.target_1_reward = input.target1Reward;
          if (input.target2Referrals !== undefined)
            mappedUpdate.target_2_referrals = input.target2Referrals;
          if (input.target2Reward !== undefined)
            mappedUpdate.target_2_reward = input.target2Reward;
          if (input.depositBkashEnabled !== undefined)
            mappedUpdate.deposit_bkash_enabled = input.depositBkashEnabled;
          if (input.depositNagadEnabled !== undefined)
            mappedUpdate.deposit_nagad_enabled = input.depositNagadEnabled;
          if (input.withdrawBkashEnabled !== undefined)
            mappedUpdate.withdraw_bkash_enabled = input.withdrawBkashEnabled;
          if (input.withdrawNagadEnabled !== undefined)
            mappedUpdate.withdraw_nagad_enabled = input.withdrawNagadEnabled;
      ...
      if (table === "advertisements") {
        const normalizeAdForBackend = (a: any) => {
          ...
          if ("userSerial" in a) {
            n.user_serial = a.userSerial;
            delete n.userSerial;
          }
          if ("durationDays" in a) {
            n.duration_days = a.durationDays;
            delete n.durationDays;
          }
          if ("transactionId" in a) {
            n.transaction_id = a.transactionId;
            delete n.transactionId;
          }
          if ("createdAt" in a) {
            n.created_at = a.createdAt;
            delete n.createdAt;
          }
          if ("expiresAt" in a) {
            n.expires_at = a.expiresAt;
            delete n.expiresAt;
          }
          if ("approvedAt" in a) {
            n.approved_at = a.approvedAt;
            delete n.approvedAt;
          }
          if ("rejectedAt" in a) {
            n.rejected_at = a.rejectedAt;
            delete n.rejectedAt;
          }
          return n;
        };
        if (method === "insert" && args?.[0])
          args[0] = Array.isArray(args[0])
            ? args[0].map(normalizeAdForBackend)
            : normalizeAdForBackend(args[0]);
        if ((method === "update" || method === "upsert") && args?.[0])
          args[0] = normalizeAdForBackend(args[0]);
        if (dbEq && dbEq[0] === "userId") dbEq[0] = "user_id";
        if (dbEqs)
          dbEqs.forEach((r: any) => {
            if (r && r[0] === "userId") r[0] = "user_id";
          });
      }
      ...
      if (table === "tickets") {
        const normalizeTicketForBackend = (t: any) => {
          ...
          if ("userSerial" in t) {
            n.user_serial = t.userSerial;
            delete n.userSerial;
          }
          if ("adminReply" in t) {
            n.admin_reply = t.adminReply;
            delete n.adminReply;
          }
          if ("createdAt" in t) {
        ...
        if (method === "insert" && args?.[0])
          args[0] = Array.isArray(args[0])
            ? args[0].map(normalizeTicketForBackend)
            : normalizeTicketForBackend(args[0]);
        if ((method === "update" || method === "upsert") && args?.[0])
          args[0] = normalizeTicketForBackend(args[0]);
        if (dbEq && dbEq[0] === "userId") dbEq[0] = "user_id";
        if (dbEqs)
          dbEqs.forEach((r: any) => {
            if (r && r[0] === "userId") r[0] = "user_id";
      ...
      if (table === "transactions") {
        const normalizeTxForBackend = (t: any) => {
          ...
          if ("type" in t) {
            ...
            if (typeVal === "payment" || typeVal === "ad_purchase")
              typeVal = "spend";
          ...
          if ("payment_method" in t)
            normalized.payment_method = t.payment_method;
        ...
        if (method === "update" && args?.[0]) {
          ...
          if (dbEq && dbEq[0] === "id") {
            try {
              ...
              console.error(
                "Error merging existing payment_details on update",
                e,
              );
            }
          }
        ...
        if (dbEqs && dbEqs.length > 0) {
          dbEqs = dbEqs.map((rule) => {
            if (rule && rule[0] === "type") {
              ...
              if (newVal === "payment" || newVal === "ad_purchase")
                newVal = "spend";
              return ["type", newVal];
            }
        ...
        if (dbEq && dbEq[0] === "type") {
          ...
          if (dbEq[1] === "payment" || dbEq[1] === "ad_purchase")
            dbEq[1] = "spend";
        }
        ...
        if (dbMatch && typeof dbMatch === "object") {
          ...
          if (dbMatch.type === "payment" || dbMatch.type === "ad_purchase")
            dbMatch.type = "spend";
          if (dbMatch.userId) {
      ...
      if (table === "jobs") {
        const normalizeJobForBackend = (j: any, isInsert = false) => {
          ...
          const slotsFilled =
            j.completedCount !== undefined
              ? j.completedCount
              : j.approvedCount !== undefined
                ? j.approvedCount
                : j.slots_filled !== undefined
                  ? j.slots_filled
                  : undefined;
          ...
          if (j.posterName !== undefined)
            categoryData.posterName = j.posterName;
          if (j.posterSerial !== undefined)
            categoryData.posterSerial = j.posterSerial;
          if (j.thumbnail !== undefined) categoryData.thumbnail = j.thumbnail;
          if (j.screenshotCount !== undefined)
            categoryData.screenshotCount = j.screenshotCount;
          if (j.textProofInstruction !== undefined)
            categoryData.textProofInstruction = j.textProofInstruction;
          if (j.screenshotProofInstruction !== undefined)
            categoryData.screenshotProofInstruction =
              j.screenshotProofInstruction;
          if (j.screenshotProofInstructions !== undefined)
            categoryData.screenshotProofInstructions =
              j.screenshotProofInstructions;
          if (j.requireTextProof !== undefined)
            categoryData.requireTextProof = j.requireTextProof;
          if (j.autoApprove !== undefined)
            categoryData.autoApprove = j.autoApprove;
          if (j.pinCode !== undefined) categoryData.pinCode = j.pinCode;
          if (j.pricePerWork !== undefined)
            categoryData.pricePerWork = Number(j.pricePerWork);
          if (j.maxWorkers !== undefined)
            categoryData.maxWorkers = Number(j.maxWorkers);
          if (j.completedCount !== undefined)
            categoryData.completedCount = Number(j.completedCount);
          if (j.pendingCount !== undefined)
            categoryData.pendingCount = Number(j.pendingCount);
          if (j.approvedCount !== undefined)
            categoryData.approvedCount = Number(j.approvedCount);
          if (j.isFull !== undefined) categoryData.isFull = j.isFull;
          if (j.createdAt !== undefined) categoryData.createdAt = j.createdAt;
          if (j.totalBudget !== undefined)
            categoryData.totalBudget = Number(j.totalBudget);
          if (j.serviceCharge !== undefined)
            categoryData.serviceCharge = Number(j.serviceCharge);
          if (j.grandTotal !== undefined)
            categoryData.grandTotal = Number(j.grandTotal);
        ...
        if (method === "insert" && args?.[0]) {
          ...
          for (const jobItem of tArgs) {
            ...
            const pricePerWork = Number(
              jobItem.pricePerWork || jobItem.reward || 0,
            );
            ...
            try {
              const { data } = await supabase
                .from("system_configuration")
                .select("job_service_charge")
                .eq("id", 1)
                .maybeSingle();
              if (data && data.job_service_charge !== undefined)
                feePercent = Number(data.job_service_charge);
            } catch (e) {}
            ...
            if (currentDeposit < grandTotal) {
              return res
                .status(400)
                .json({
                  error: `পোস্ট বাজেট (BDT ${grandTotal.toFixed(2)}) এর জন্য আপনার পর্যাপ্ত ডিপোজিট ব্যালেন্স নেই।`,
                });
            }
            ...
            const { error: balanceErr } = await supabase
              ...
              .update({
                ...
                heldBalance:
                  Number(posterProfile.heldBalance || 0) + totalBudget,
              })
              .eq("id", posterId);
            ...
            if (balanceErr) {
              return res
                .status(500)
                .json({
                  error: "Balance deduction failed: " + balanceErr.message,
                });
            }
          ...
          const normalizedArgs = tArgs.map((jItem) =>
            normalizeJobForBackend(jItem, true),
          );
          args[0] = Array.isArray(args[0]) ? normalizedArgs : normalizedArgs[0];
        }
        if (method === "update" && args?.[0]) {
          ...
          if (dbEq && dbEq[0] === "id") {
            ...
            if (existingJobRow) {
              ...
              try {
                if (
                  existingJobRow.category &&
                  (existingJobRow.category.startsWith("{") ||
                    existingJobRow.category.startsWith("["))
                ) {
              ...
              // Transition to 'deleted' or 'rejected' from pending (Reject)
                ...
                (updatedJob.status === "deleted" ||
                  updatedJob.status === "rejected") &&
                oldStatus === "pending"
              ) {
                // Refund poster
                const posterId =
                  jobCategory.posterId || existingJobRow.author_id;
                const totalBudget = Number(
                  jobCategory.totalBudget ||
                    Number(existingJobRow.reward) *
                      Number(existingJobRow.slots),
                );
                const grandTotal = Number(
                  jobCategory.grandTotal || totalBudget + totalBudget * 0.1,
                );
                ...
                if (posterProfile) {
                  await supabase
                    ...
                    .update({
                      ...
                      depositBalance:
                        Number(posterProfile.depositBalance || 0) + grandTotal,
                    })
                    .eq("id", posterId);
                }
              ...
              // Transition to 'deleted' from active 'open' (Cancel/Delete)
                ...
                updatedJob.status === "deleted" &&
                (oldStatus === "open" || oldStatus === "delete_requested")
              ) {
                const posterId =
                  jobCategory.posterId || existingJobRow.author_id;
                const maxWorkers = Number(
                  existingJobRow.slots || jobCategory.maxWorkers || 1,
                );
                const approvedCount = Number(
                  existingJobRow.slots_filled || jobCategory.approvedCount || 0,
                );
                const pendingCount = Number(jobCategory.pendingCount || 0);
                const remainingCount =
                  maxWorkers - (approvedCount + pendingCount);
                const pricePerWork = Number(
                  existingJobRow.reward || jobCategory.pricePerWork || 0,
                );
                const refundAmount =
                  remainingCount > 0 ? remainingCount * pricePerWork : 0;
                  ...
                  if (posterProfile) {
                    await supabase
                      ...
                      .update({
                        heldBalance: Math.max(
                          0,
                          Number(posterProfile.heldBalance || 0) - refundAmount,
                        ),
                        depositBalance:
                          Number(posterProfile.depositBalance || 0) +
                          refundAmount,
                      })
                      .eq("id", posterId);
                  }
          ...
          if (dbEq && dbEq[0] === "id") {
            try {
              ...
              if (existingJob && existingJob.category) {
                let parsedCategory = {};
                try {
                  parsedCategory = JSON.parse(existingJob.category);
                } catch (catErr) {}
                const newCategory = {
                  ...parsedCategory,
        ...
        if (dbOrder && dbOrder[0] === "pricePerWork") {
          dbOrder = ["reward", dbOrder[1]];
        }
      }
        ...
        const normalizeSubmissionForBackend = (s: any, isInsert = false) => {
          ...
          if (s.proof !== undefined) {
            if (
              ...
              (s.proof.startsWith("{") || s.proof.startsWith("["))
            ) {
              try {
          ...
          if (s.screenshots !== undefined)
            proofData.screenshots = s.screenshots;
          if (s.rejectionReason !== undefined)
            proofData.rejectionReason = s.rejectionReason;
          if (s.workerName !== undefined) proofData.workerName = s.workerName;
          if (s.workerSerial !== undefined)
            proofData.workerSerial = s.workerSerial;
        ...
        if (method === "insert" && args?.[0]) {
          ...
          for (const subItem of tArgs) {
            ...
            if (jobErr || !jobData) {
              return res
                .status(404)
                .json({ error: "This job no longer exists." });
            }
            ...
            if (status !== "open") {
              return res
                .status(400)
                .json({ error: "This job is no longer active." });
            }

            const maxWorkers = Number(jobData.slots || extra.maxWorkers || 1);
            const approvedCount = Number(
              jobData.slots_filled || extra.approvedCount || 0,
            );
            ...
            if (approvedCount + pendingCount >= maxWorkers || extra.isFull) {
              return res
                ...
                .json({
                  error:
                    "দুঃখিত! আপনি সাবমিট করার আগেই এই জবের সব স্লট বুক হয়ে গেছে।",
                });
            }
            ...
            if (existingSub) {
              return res
                .status(400)
                .json({
                  error: "You have already submitted proof for this job.",
                });
            }
            ...
            const isAutoApprove =
              extra.autoApprove === true || extra.autoApprove === "true";
            if (isAutoApprove) {
            ...
            const newApproved = isAutoApprove
              ? approvedCount + 1
              : approvedCount;
            ...
            if (workerProfile) {
              if (isAutoApprove) {
                await supabase
                  ...
                  .update({
                    earningBalance:
                      Number(workerProfile.earningBalance || 0) + reward,
                  })
                  .eq("id", workerId);
                ...
                if (posterProfile) {
                  await supabase
                    ...
                    .update({
                      heldBalance: Math.max(
                        0,
                        Number(posterProfile.heldBalance || 0) - reward,
                      ),
                    })
                    .eq("id", posterId);
                }
                ...
                await supabase
                  .from("profiles")
                  .update({
                    pendingEarningBalance:
                      Number(workerProfile.pendingEarningBalance || 0) + reward,
                  })
                  .eq("id", workerId);
              }
            }
          }
          const normalizedArgs = tArgs.map((sItem) =>
            normalizeSubmissionForBackend(sItem, true),
          );
          args[0] = Array.isArray(args[0]) ? normalizedArgs : normalizedArgs[0];
        }
        if (method === "update" && args?.[0]) {
          ...
          if (
            ...
            if (idVal) {
              ...
              if (submission && submission.status === "pending") {
                ...
                const reward = Number(
                  subProofExtra.reward || submission.reward || 0,
                );
                ...
                if (jobData) {
                  ...
                  const maxWorkers = Number(
                    jobData.slots || jobCategory.maxWorkers || 1,
                  );
                  const currentPending = Number(jobCategory.pendingCount || 0);
                  const currentApproved = Number(
                    jobData.slots_filled || jobCategory.approvedCount || 0,
                  );
                  ...
                  if (updatedSub.status === "approved") {
                    ...
                    if (workerProfile) {
                      await supabase
                        ...
                        .update({
                          earningBalance:
                            Number(workerProfile.earningBalance || 0) + reward,
                          pendingEarningBalance: Math.max(
                            0,
                            Number(workerProfile.pendingEarningBalance || 0) -
                              reward,
                          ),
                        })
                        .eq("id", submission.worker_id);
                      validateReferral(submission.worker_id).catch(
                        console.error,
                      );
                    }
                    ...
                    const isJobFullNow =
                      newPending + currentApproved >= maxWorkers;
                    ...
                    const jobStatus =
                      jobCategory.status || jobData.status || "open";
                  ...
                  subProofExtra.rejectionReason =
                    updatedSub.rejectionReason ||
                    updatedSub.reject_reason ||
                    "";
          ...
          if (dbEq && dbEq[0] === "id") {
            try {
              ...
              if (existingSub && existingSub.proof) {
                let parsedProof = {};
                try {
                  parsedProof = JSON.parse(existingSub.proof);
                } catch (proofErr) {}
                const newProof = {
                  ...parsedProof,
        ...
        if (dbEqs && dbEqs.length > 0) {
          dbEqs = dbEqs.filter((rule) => {
            if (rule && rule[0] === "jobId") {
              ...
              memFilter = (subItem: any) =>
                (!oldF || oldF(subItem)) && subItem.posterId === pId;
              return false;
            }
            return true;
          });
      ...
      if (dbNotIn)
        queryBuilder = queryBuilder.not(
          dbNotIn[0],
          "in",
          `(${dbNotIn[1].map((x: any) => `"${x}"`).join(",")})`,
        );
      if (dbOr) queryBuilder = queryBuilder.or(dbOr);
      if (ilike) queryBuilder = queryBuilder.ilike(ilike[0], ilike[1]);
      ...
      if (table === "jobs" && data) {
        ...
        const normalizeJobForFrontend = (j: any) => {
          ...
          return {
            ...
            screenshotProofInstructions:
              extra.screenshotProofInstructions || [],
            requireTextProof:
  ...
  app.post("/api/admin/update-user", async (req, res) => {
    try {
      ...
      if (!targetUserId || !updates)
        return res
          .status(400)
          .json({ error: "Missing target user id or updates" });
      ...
      if (profileFetchErr || !existingProfile) {
        return res
          ...
          .json({
            error:
              "Target user profile not found in database. (ব্যবহারকারীর প্রোফাইল ডাটাবেজে পাওয়া যায়নি।)",
          });
      }
      ...
      const isPhoneChanged =
        ...
        updates.phone !== (existingProfile.phone || "");
      const isDisplayNameChanged =
        updates.displayName !== undefined &&
        updates.displayName !==
          (existingProfile.displayName || existingProfile.name || "");
      ...
      if (needsAuthUpdate) {
        ...
        const isServiceKeyMissing =
          !process.env.SUPABASE_SERVICE_KEY &&
          !process.env.SUPABASE_SERVICE_ROLE_KEY;
        if (isServiceKeyMissing) {
          return res.status(400).json({
            error:
              "মাস্টার সার্ভার কি বা SUPABASE_SERVICE_ROLE_KEY আপনার AI Studio Settings ➜ Secrets প্যানেলে সেট করা নেই। পাসওয়ার্ড, ইমেল বা তথ্য পরিবর্তন করতে এটি যুক্ত করা বাধ্যতামূলক।",
          });
        }

        try {
          const { error: authUpdateError } =
            await supabase.auth.admin.updateUserById(targetUserId, authUpdates);
          if (authUpdateError) {
      ...
      if (isPhoneChanged)
        profileUpdates.phone = updates.phone ? updates.phone.trim() : null;
      ...
      if (Object.keys(profileUpdates).length > 0) {
        ...
        if (profileUpdateError) {
          return res
            ...
            .json({
              error:
                "ডাটাবেজ প্রোফাইল আপডেট ব্যর্থ হয়েছে: " +
                profileUpdateError.message,
              diag: { existingProfile, updates, profileUpdates },
            });
        }
      }
  ...
  app.post("/api/admin/impersonate", express.json(), async (req, res) => {
    try {
      ...
      if (!targetUserId)
        return res.status(400).json({ error: "No target user provided" });
      ...
      const isMaster = [
        ...
        "harunurrashid93427@gmail.com",
        "harunbhai2728@gmail.com",
      ].includes(adminUser.email?.toLowerCase() || "");
      if (profile?.role !== "admin" && !isMaster) {
      ...
      if (targetError || !targetProfile?.email) {
        return res
          .status(404)
          .json({ error: "Target user or email not found" });
      }
    ...
    async (req, res) => {
      try {
        ...
        if (!adminId) {
          return res
            .status(401)
            .json({ error: "No active impersonation session found" });
        }
        ...
        if (targetJwt) {
          try {
            ...
            console.warn(
              "Could not gracefully sign out target user on backend:",
              e,
            );
          }
        }
        ...
        const { data: linkData, error: linkError } =
          await supabase.auth.admin.generateLink({
            ...
            options: {
              redirectTo:
                req.body.redirectTo ||
                `${req.headers.origin || "http://localhost:3000"}/admin`,
            },
          });
        ...
        if (linkError) {
          return res
            ...
            .json({
              error:
                "Failed to generate admin restore link: " + linkError.message,
            });
        }
  ...
  app.get("/api/admin/data", async (req, res) => {
    try {
      ...
      if (authErr || !user) {
        console.log(
          "Supabase token getUser was not resolved by SDK, using local token decoder fallback.",
        );
        try {
          const base64Url = token.split(".")[1];
          const base64 = base64Url.replace(/-/g, "+").replace(/_/g, "/");
    ...
    try {
      ...
      if (hasSupabaseRedeemTables) {
        try {
          ...
          console.warn(
            "Supabase redeem_codes select failed, using fallback:",
            e,
          );
        }
      }
        ...
        try {
          ...
          if (!error && data && data.length > 0) {
            ...
            if (error.code === "23505") {
              return res
                .status(400)
                .json({ error: "পেনেল অনুযায়ী এই কোড ইতিমধ্যে তৈরি করা আছে!" });
            }
          }
        } catch (e: any) {
          console.warn(
            "Supabase redeem_codes insert failed, fallback schema:",
            e,
          );
        }
      }
    ...
    try {
      ...
      if (hasSupabaseRedeemTables) {
        try {
          ...
          console.warn(
            "Supabase redeem_codes deletion error, checking fallback:",
            e,
          );
        }
      }
      ...
      const codes = getRedeemCodesFromStore().filter(
        (c: any) => c.id !== id && c.code !== id,
      );
      const usages = getRedeemCodeUsagesFromStore().filter(
        (u: any) => u.redeem_code_id !== id,
      );
      saveRedeemCodesToStore(codes);
  ...
  app.post("/api/redeem-code/claim", async (req, res) => {
    ...
    try {
      ...
      if (!user)
        return res
          .status(401)
          .json({ error: "সেশন শেষ হয়েছে, দয়া করে আবার লগইন করুন।" });

      let { code } = req.body;
      if (!code) {
        return res
          .status(400)
          .json({ error: "রিডিম কোডটি খালি রাখা যাবে না!" });
      }
      ...
      if (activeProxyLocks.has(claimLockKey)) {
        return res
          ...
          .json({
            error:
              "এই রিডিম কোডটি ইতিমধ্যেই প্রসেস হচ্ছে, দয়া করে একটু অপেক্ষা করুন।",
          });
      }
      ...
      if (usedCount >= maxUses) {
        return res
          .status(400)
          .json({ error: "This redeem code has expired/reached its limit!" });
      }
      ...
      if (!alreadyClaimed) {
        ...
        alreadyClaimed = localUsages.some(
          (u: any) =>
            u.redeem_code_id === activeCode.id && u.user_id === user.id,
        );
      }
      ...
      if (alreadyClaimed) {
        return res
          .status(400)
          .json({ error: "You have already redeemed this code once!" });
      }
      ...
      if (profileErr || !userProfile) {
        return res
          .status(400)
          .json({ error: "User profile not found in system" });
      }
      ...
      if (balanceErr) {
        return res
          ...
          .json({
            error:
              "ব্যব্যালেন্স আপডেট করতে সমস্যা হয়েছে: " + balanceErr.message,
          });
      }
      ...
      const localCodes = getRedeemCodesFromStore();
      const codeIndex = localCodes.findIndex(
        (c: any) => c.code === formattedCode,
      );
      if (codeIndex !== -1) {
        localCodes[codeIndex].used_count =
          (localCodes[codeIndex].used_count || 0) + 1;
    ...
    try {
      ...
      if (hasSupabaseRedeemTables) {
        try {
          ...
          console.warn(
            "Supabase redeem_code_usages join failed, falling back to local storage join:",
            e,
          );
        }
      }
      ...
      const historyList = usages.map((usage: any) => {
        const codeDetails = codes.find(
          (c: any) =>
            c.id === usage.redeem_code_id || c.code === usage.redeem_code_id,
        );
        return {
          id: usage.id,
  ...
  app.get("/api/settings/deposit-rules", async (req, res) => {
    try {
      const defaultRules =
        "উক্ত নাম্বারে টাকা পাঠিয়ে সেন্ডার নাম্বার, টাকার পরিমান ও ট্রানজেকশন আইডি দিন। ভুয়া রিকোয়েস্ট দিলে একাউন্ট ব্লক করা হবে।";
      ...
      if (hasSupabaseSettingsTable) {
        try {
          ...
          console.warn(
            "Supabase system_settings SELECT failed, falling back to local:",
            e,
          );
        }
      }
  ...
  app.put("/api/settings/deposit-rules", async (req, res) => {
    try {
      ...
      if (rules === undefined)
        return res.status(400).json({ error: "Missing rules content" });
      ...
      if (hasSupabaseSettingsTable) {
        try {
          ...
          console.warn(
            "Supabase system_settings upsert/update failed, saving locally:",
            e,
          );
        }
      }
  ...
  app.get("/api/settings/withdraw-rules", async (req, res) => {
    try {
      const defaultRules =
        "নম্বরটি ভালোভাবে চেক করুন। ভুল নম্বরে টাকা গেলে কর্তৃপক্ষ দায়ী নয়। পেমেন্ট সম্পন্ন হতে ১-২৪ ঘণ্টা সময় লাগতে পারে।";
      ...
      if (hasSupabaseSettingsTable) {
        try {
          ...
          console.warn(
            "Supabase system_settings SELECT failed for withdraw_rules, falling back to local:",
            e,
          );
        }
      }
    ...
    try {
      ...
      if (hasSupabaseSettingsTable) {
        try {
          ...
          console.warn(
            "Supabase system_settings upsert/update failed for withdraw_rules, saving locally:",
            e,
          );
        }
      }
  ...
  app.get("/api/settings/ad-post-rules", async (req, res) => {
    try {
      const defaultRules =
        "সতর্কতা: আপনি অ্যাড পোস্ট করার পর যদি আবার অ্যাড ডিলিট করেন, তাহলে কোনো রিফান্ড পাবেন না।";
      ...
      if (hasSupabaseSettingsTable) {
        try {
          ...
          console.warn(
            "Supabase system_settings SELECT failed for ad_post_rules, falling back to local:",
            e,
          );
        }
      }
    ...
    try {
      ...
      if (hasSupabaseSettingsTable) {
        try {
          ...
          console.warn(
            "Supabase system_settings upsert/update failed for ad_post_rules, saving locally:",
            e,
          );
        }
      }
  ...
  app.post("/api/job/edit", async (req, res) => {
    try {
      ...
      const { jobId, title, description, newPricePerWork, newMaxWorkers } =
        req.body;
      ...
      if (
        ...
        !title ||
        !description ||
        newPricePerWork === undefined ||
        newMaxWorkers === undefined
      ) {
        return res.status(400).json({ error: "Missing required fields" });
      }
      ...
      if (job.author_id !== user.id) {
        return res
          .status(403)
          .json({ error: "You are not the author of this job" });
      }
      ...
      if (newPrice < oldPrice) {
        return res
          .status(400)
          .json({ error: "You can only increase the price per work" });
      }
      if (newWorkers < oldWorkers) {
        return res
          .status(400)
          .json({ error: "You can only increase the number of workers" });
      }
      ...
      if (extraGrandTotal > 0) {
        const { data: profile } = await supabase
          .from("profiles")
          .select("depositBalance, heldBalance")
          .eq("id", user.id)
          .single();
        if (!profile)
        ...
        if (Number(profile.depositBalance) < extraGrandTotal) {
          return res
            .status(400)
            .json({
              error: `You need ${extraGrandTotal.toFixed(2)} BDT additional deposit balance.`,
            });
        }
      ...
      if (
        (job.status === "completed" || job.status === "full") &&
