...
export function JobDetails() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (job.screenshotCount > 0 && screenshotUrls.length < job.screenshotCount) {
          ...
          <form onSubmit={handleSubmit} className="bg-gray-900 p-8 md:p-12 rounded-[2.5rem] text-white space-y-8 shadow-2xl">
            ...
            <div className="space-y-6 text-left">
              {job.requireTextProof && (
                <div className="space-y-3">
                  ...
                  <textarea
                    required
                    rows={4}
                    placeholder="Type the required information here..."
              ...
              {job.screenshotCount > 0 && (
                <div className="space-y-4">
                  <label className="text-sm font-black uppercase tracking-widest text-gray-400">
                    Upload {job.screenshotCount} Required Screenshot(s)
                  </label>
                  {Array.from({ length: job.screenshotCount }).map((_, i) => (
                    <div key={i} className="bg-white/5 border border-white/10 rounded-2xl p-4 border-dashed hover:border-white/20 transition-all flex items-center gap-4">
